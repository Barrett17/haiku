...
BMediaClient::SendBuffer(BMediaConnection* connection, BBuffer* buffer)
{
	...
	//return fNode->SendBuffer(buffer, connection->Destination());
	return B_ERROR;
}
	...
	BMediaConnection* source)
{
	...
	//BMediaConnection* conn = new BMediaConnection(this, ret);
	return B_ERROR;
}
	...
	BMediaConnection* output)
{
	...
	if (connection->Source() == media_source::null)
		return B_MEDIA_BAD_SOURCE;

	media_input ourInput;
	media_output theirOutput;
	media_format format = connection->AcceptedFormat();

	connection->BuildMediaInput(&ourInput);
	connection->BuildMediaOutput(&theirOutput);
